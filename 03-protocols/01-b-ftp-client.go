...
func main() {
	...
	for {
		...
		// split into <CMD> <ARG> form
		strs := strings.SplitN(line, " ", 2)

		// translation from UI to protocol
		switch strs[0] {
		case uiDir:
			dirRequest(conn)
		case uiCd:
...
func dirRequest(conn net.Conn) {
	...
	for {
		// read until blank line (or up to 512 bytes)
		n, _ := conn.Read(buf[0:])
		...
		// else, keep reading line by line
	}
}
...
func cdRequest(conn net.Conn, dir string) {
	...
	// read until blank line or 512 bytes
